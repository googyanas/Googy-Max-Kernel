...
static _maliggy_mem_validation_t maliggy_mem_validator = { MALI_INVALID_MEM_ADDR, MALI_INVALID_MEM_ADDR };

_maliggy_osk_errcode_t maliggy_mem_validation_add_range(u32 start, u32 size)
{
	/* MALI_SEC */
#if defined(MALI_SEC_MEM_VALIDATION)
	...
	if (MALI_INVALID_MEM_ADDR != maliggy_mem_validator.phys_base)
	{
	...
	maliggy_mem_validator.phys_base = start;
	maliggy_mem_validator.size = size;
	MALI_DEBUG_PRINT(2, ("Memory Validator installed for Mali physical address base=0x%08X, size=0x%08X\n",
	                 maliggy_mem_validator.phys_base, maliggy_mem_validator.size));

	return _MALI_OSK_ERR_OK;
}

_maliggy_osk_errcode_t maliggy_mem_validation_check(u32 phys_addr, u32 size)
{
	if (phys_addr < (phys_addr + size)) /* Don't allow overflow (or zero size) */
	{
		if ((0 == ( phys_addr & (~_MALI_OSK_CPU_PAGE_MASK))) &&
			(0 == ( size & (~_MALI_OSK_CPU_PAGE_MASK))))
		{
			if ((phys_addr          >= maliggy_mem_validator.phys_base) &&
				((phys_addr + (size - 1)) >= maliggy_mem_validator.phys_base) &&
				(phys_addr          <= (maliggy_mem_validator.phys_base + (maliggy_mem_validator.size - 1))) &&
				((phys_addr + (size - 1)) <= (maliggy_mem_validator.phys_base + (maliggy_mem_validator.size - 1))) )
			{
