...
static maliggy_bool maliggy_pp_scheduler_is_suspended(void);
static void maliggy_pp_scheduler_do_schedule(void *arg);
#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
static void maliggy_pp_scheduler_do_job_delete(void *arg);
#endif
static void maliggy_pp_scheduler_job_queued(void);
static void maliggy_pp_scheduler_job_completed(void);
...
static struct maliggy_group *virtual_group = NULL;                 /* Virtual group (if any) */
static enum
{
	VIRTUAL_GROUP_IDLE,
...
static _maliggy_osk_wait_queue_t *pp_scheduler_working_wait_queue = NULL;
static u32 pause_count = 0;

static _maliggy_osk_lock_t *pp_scheduler_lock = NULL;
/* Contains tid of thread that locked the scheduler or 0, if not locked */
MALI_DEBUG_CODE(static u32 pp_scheduler_lock_owner = 0);

static _maliggy_osk_wq_work_t *pp_scheduler_wq_schedule = NULL;

#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
static _maliggy_osk_wq_work_t *pp_scheduler_wq_job_delete = NULL;
static _maliggy_osk_lock_t *pp_scheduler_job_delete_lock = NULL;
static _MALI_OSK_LIST_HEAD_STATIC_INIT(pp_scheduler_job_deletion_queue);
#endif

MALI_STATIC_INLINE maliggy_bool maliggy_pp_scheduler_has_virtual_group(void)
{
...
_maliggy_osk_errcode_t maliggy_pp_scheduler_initialize(void)
{
	...
	pp_scheduler_lock = _maliggy_osk_lock_init(lock_flags, 0, _MALI_OSK_LOCK_ORDER_SCHEDULER);
	if (NULL == pp_scheduler_lock)
	{
		return _MALI_OSK_ERR_NOMEM;
	}

	pp_scheduler_working_wait_queue = _maliggy_osk_wait_queue_init();
	if (NULL == pp_scheduler_working_wait_queue)
	{
		_maliggy_osk_lock_term(pp_scheduler_lock);
		return _MALI_OSK_ERR_NOMEM;
	}

	pp_scheduler_wq_schedule = _maliggy_osk_wq_create_work(maliggy_pp_scheduler_do_schedule, NULL);
	if (NULL == pp_scheduler_wq_schedule)
	{
		_maliggy_osk_wait_queue_term(pp_scheduler_working_wait_queue);
		_maliggy_osk_lock_term(pp_scheduler_lock);
		return _MALI_OSK_ERR_NOMEM;
	}

#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
	pp_scheduler_wq_job_delete = _maliggy_osk_wq_create_work(maliggy_pp_scheduler_do_job_delete, NULL);
	if (NULL == pp_scheduler_wq_job_delete)
	{
		_maliggy_osk_wq_delete_work(pp_scheduler_wq_schedule);
		_maliggy_osk_wait_queue_term(pp_scheduler_working_wait_queue);
		_maliggy_osk_lock_term(pp_scheduler_lock);
		return _MALI_OSK_ERR_NOMEM;
	}

	pp_scheduler_job_delete_lock = _maliggy_osk_lock_init(_MALI_OSK_LOCKFLAG_ORDERED | _MALI_OSK_LOCKFLAG_SPINLOCK_IRQ |_MALI_OSK_LOCKFLAG_NONINTERRUPTABLE, 0, _MALI_OSK_LOCK_ORDER_SCHEDULER_DEFERRED);
	if (NULL == pp_scheduler_job_delete_lock)
	{
		_maliggy_osk_wq_delete_work(pp_scheduler_wq_job_delete);
		_maliggy_osk_wq_delete_work(pp_scheduler_wq_schedule);
		_maliggy_osk_wait_queue_term(pp_scheduler_working_wait_queue);
		_maliggy_osk_lock_term(pp_scheduler_lock);
		return _MALI_OSK_ERR_NOMEM;
...
{
#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
	_maliggy_osk_lock_term(pp_scheduler_job_delete_lock);
	_maliggy_osk_wq_delete_work(pp_scheduler_wq_job_delete);
#endif
...
void maliggy_pp_scheduler_populate(void)
{
	...
	num_groups = maliggy_group_get_glob_num_groups();

	/* Do we have a virtual group? */
	for (i = 0; i < num_groups; i++)
	{
		group = maliggy_group_get_glob_group(i);
	...
	for (i = 0; i < num_groups; i++)
	{
		...
		if (NULL != pp_core && !maliggy_group_is_virtual(group))
		{
			if (0 == pp_version)
			{
				/* Retrieve PP version from the first available PP core */
				pp_version = maliggy_pp_core_get_version(pp_core);
			}

			if (maliggy_pp_scheduler_has_virtual_group())
			{
				/* Add all physical PP cores to the virtual group */
				maliggy_group_lock(virtual_group);
				group->state = MALI_GROUP_STATE_JOINING_VIRTUAL;
				maliggy_group_add_group(virtual_group, group, MALI_TRUE);
				maliggy_group_unlock(virtual_group);
			}
			else
			{
				_maliggy_osk_list_add(&group->pp_scheduler_list, &group_list_idle);
			}
...
{
	struct maliggy_group *group, *temp;

	MALI_DEBUG_ASSERT(_maliggy_osk_list_empty(&group_list_working));
	MALI_DEBUG_ASSERT(VIRTUAL_GROUP_WORKING != virtual_group_state);

	/* Delete all groups owned by scheduler */
	if (maliggy_pp_scheduler_has_virtual_group())
	{
		maliggy_group_delete(virtual_group);
	}

	_MALI_OSK_LIST_FOREACHENTRY(group, temp, &group_list_idle, struct maliggy_group, pp_scheduler_list)
	{
		maliggy_group_delete(group);
	}
	_MALI_OSK_LIST_FOREACHENTRY(group, temp, &group_list_disabled, struct maliggy_group, pp_scheduler_list)
	{
		maliggy_group_delete(group);
	}
}

MALI_STATIC_INLINE void maliggy_pp_scheduler_lock(void)
{
	if(_MALI_OSK_ERR_OK != _maliggy_osk_lock_wait(pp_scheduler_lock, _MALI_OSK_LOCKMODE_RW))
	{
	...
	MALI_DEBUG_CODE(pp_scheduler_lock_owner = _maliggy_osk_get_tid());
}

MALI_STATIC_INLINE void maliggy_pp_scheduler_unlock(void)
{
	MALI_DEBUG_PRINT(5, ("Mali PP scheduler: Releasing PP scheduler lock\n"));
	MALI_DEBUG_ASSERT(_maliggy_osk_get_tid() == pp_scheduler_lock_owner);
	MALI_DEBUG_CODE(pp_scheduler_lock_owner = 0);
	_maliggy_osk_lock_signal(pp_scheduler_lock, _MALI_OSK_LOCKMODE_RW);
}

MALI_STATIC_INLINE void maliggy_pp_scheduler_disable_empty_virtual(void)
{
	MALI_ASSERT_GROUP_LOCKED(virtual_group);

	if (maliggy_group_virtual_disable_if_empty(virtual_group))
	{
...
MALI_STATIC_INLINE void maliggy_pp_scheduler_enable_empty_virtual(void)
{
	MALI_ASSERT_GROUP_LOCKED(virtual_group);

	if (maliggy_group_virtual_enable_if_empty(virtual_group))
	{
...
#ifdef DEBUG
MALI_STATIC_INLINE void maliggy_pp_scheduler_assert_locked(void)
{
	MALI_DEBUG_ASSERT(_maliggy_osk_get_tid() == pp_scheduler_lock_owner);
}
#define MALI_ASSERT_PP_SCHEDULER_LOCKED() maliggy_pp_scheduler_assert_locked()
#else
 ...
 */
MALI_STATIC_INLINE struct maliggy_pp_job *maliggy_pp_scheduler_get_physical_job(void)
{
	MALI_ASSERT_PP_SCHEDULER_LOCKED();

	if (!_maliggy_osk_list_empty(&job_queue))
	{
		struct maliggy_pp_job *job;

		MALI_DEBUG_ASSERT(job_queue_depth > 0);
		job = _MALI_OSK_LIST_ENTRY(job_queue.next, struct maliggy_pp_job, list);

		if (!maliggy_pp_job_has_active_barrier(job))
		{
			return job;
		}
	}
...
MALI_STATIC_INLINE void maliggy_pp_scheduler_dequeue_physical_job(struct maliggy_pp_job *job)
{
	MALI_ASSERT_PP_SCHEDULER_LOCKED();
	MALI_DEBUG_ASSERT(job_queue_depth > 0);

	/* Remove job from queue */
	if (!maliggy_pp_job_has_unstarted_sub_jobs(job))
	{
		/* All sub jobs have been started: remove job from queue */
		_maliggy_osk_list_delinit(&job->list);
	}
 ...
 */
MALI_STATIC_INLINE struct maliggy_pp_job *maliggy_pp_scheduler_get_virtual_job(void)
{
	MALI_ASSERT_PP_SCHEDULER_LOCKED();
	MALI_DEBUG_ASSERT_POINTER(virtual_group);

	if (!_maliggy_osk_list_empty(&virtual_job_queue))
	{
		struct maliggy_pp_job *job;

		MALI_DEBUG_ASSERT(virtual_job_queue_depth > 0);
		job = _MALI_OSK_LIST_ENTRY(virtual_job_queue.next, struct maliggy_pp_job, list);
...
MALI_STATIC_INLINE void maliggy_pp_scheduler_dequeue_virtual_job(struct maliggy_pp_job *job)
{
	MALI_ASSERT_PP_SCHEDULER_LOCKED();
	MALI_DEBUG_ASSERT(virtual_job_queue_depth > 0);
 ...
 */
MALI_STATIC_INLINE maliggy_bool maliggy_pp_scheduler_can_move_virtual_to_physical(void)
{
	MALI_ASSERT_PP_SCHEDULER_LOCKED();
	MALI_DEBUG_ASSERT(maliggy_pp_scheduler_has_virtual_group());
	MALI_ASSERT_GROUP_LOCKED(virtual_group);
	/*
	...
	return (VIRTUAL_GROUP_IDLE == virtual_group_state) &&
	       _maliggy_osk_list_empty(&group_list_idle) &&
	       _maliggy_osk_list_empty(&group_list_working) &&
	       (NULL != maliggy_pp_scheduler_get_physical_job());
}

MALI_STATIC_INLINE struct maliggy_group *maliggy_pp_scheduler_acquire_physical_group(void)
{
	MALI_ASSERT_PP_SCHEDULER_LOCKED();

	if (!_maliggy_osk_list_empty(&group_list_idle))
	{
		MALI_DEBUG_PRINT(4, ("Mali PP scheduler: Acquiring physical group from idle list\n"));
		return _MALI_OSK_LIST_ENTRY(group_list_idle.next, struct maliggy_group, pp_scheduler_list);
	}
	else if (maliggy_pp_scheduler_has_virtual_group())
	{
		MALI_ASSERT_GROUP_LOCKED(virtual_group);
		if (maliggy_pp_scheduler_can_move_virtual_to_physical())
		{
			struct maliggy_group *group;
			MALI_DEBUG_PRINT(4, ("Mali PP scheduler: Acquiring physical group from virtual group\n"));
			group = maliggy_group_acquire_group(virtual_group);

			if (maliggy_pp_scheduler_has_virtual_group())
			{
				maliggy_pp_scheduler_disable_empty_virtual();
			}
...
static void maliggy_pp_scheduler_schedule(void)
{
	struct maliggy_group* physical_groups_to_start[MALI_MAX_NUMBER_OF_PP_GROUPS-1];
	struct maliggy_pp_job* physical_jobs_to_start[MALI_MAX_NUMBER_OF_PP_GROUPS-1];
	...
	maliggy_pp_scheduler_lock();
	if (pause_count > 0)
	{
		/* Scheduler is suspended, don't schedule any jobs */
		maliggy_pp_scheduler_unlock();
		if (maliggy_pp_scheduler_has_virtual_group())
		{
			maliggy_group_unlock(virtual_group);
		}
	...
	while (1)
	{
		...
		job = maliggy_pp_scheduler_get_physical_job();
		if (NULL == job)
		{
			break; /* No job, early out */
		}

		MALI_DEBUG_ASSERT(!maliggy_pp_job_is_virtual(job));
		MALI_DEBUG_ASSERT(maliggy_pp_job_has_unstarted_sub_jobs(job));
		MALI_DEBUG_ASSERT(1 <= maliggy_pp_job_get_sub_job_count(job));
		...
		group = maliggy_pp_scheduler_acquire_physical_group();
		if (NULL == group)
		{
		...
		subjob = maliggy_pp_job_get_first_unstarted_sub_job(job);
		maliggy_pp_job_mark_sub_job_started(job, subjob);

		/* Remove job from queue (if we now got the last subjob) */
		maliggy_pp_scheduler_dequeue_physical_job(job);

		/* Move group to working list */
		_maliggy_osk_list_move(&(group->pp_scheduler_list), &group_list_working);
	...
	if (maliggy_pp_scheduler_has_virtual_group())
	{
		...
		{
			struct maliggy_pp_job *job = maliggy_pp_scheduler_get_virtual_job();
			if (NULL != job)
			{
				MALI_DEBUG_ASSERT(maliggy_pp_job_is_virtual(job));
				MALI_DEBUG_ASSERT(maliggy_pp_job_has_unstarted_sub_jobs(job));
				...
				maliggy_pp_job_mark_sub_job_started(job, 0);

				/* Remove job from queue */
				maliggy_pp_scheduler_dequeue_virtual_job(job);
				...
				/* Start job */
				maliggy_group_start_pp_job(virtual_group, job, 0);
				MALI_DEBUG_PRINT(4, ("Mali PP scheduler: Virtual job %u (0x%08X) part %u/%u started (from schedule)\n",
				                     maliggy_pp_job_get_id(job), job, 1,
				                     maliggy_pp_job_get_sub_job_count(job)));
	...
	{
		struct maliggy_group *group = physical_groups_to_start[i];
		struct maliggy_pp_job *job  = physical_jobs_to_start[i];
		...
		maliggy_group_start_pp_job(group, job, sub_job);
		MALI_DEBUG_PRINT(4, ("Mali PP scheduler: Physical job %u (0x%08X) part %u/%u started (from schedule)\n",
		                     maliggy_pp_job_get_id(job), job, sub_job + 1,
		                     maliggy_pp_job_get_sub_job_count(job)));

		maliggy_group_unlock(group);

		/* remove the return value from maliggy_group_start_xx_job, since we can't fail on Mali-300++ */
	}
}

static void maliggy_pp_scheduler_return_job_to_user(struct maliggy_pp_job *job, maliggy_bool deferred)
{
	if (MALI_FALSE == maliggy_pp_job_use_no_notification(job))
	{
		u32 i;
		u32 num_counters_to_copy;
		maliggy_bool success = maliggy_pp_job_was_success(job);

		_maliggy_uk_pp_job_finished_s *jobres = job->finished_notification->result_buffer;
		_maliggy_osk_memset(jobres, 0, sizeof(_maliggy_uk_pp_job_finished_s)); /* @@@@ can be removed once we initialize all members in this struct */
		jobres->user_job_ptr = maliggy_pp_job_get_user_id(job);
		...
		if (maliggy_pp_job_is_virtual(job))
		{
			num_counters_to_copy = num_cores; /* Number of physical cores available */
		}
		else
		{
			num_counters_to_copy = maliggy_pp_job_get_sub_job_count(job);
		}

		for (i = 0; i < num_counters_to_copy; i++)
		{
			jobres->perf_counter0[i] = maliggy_pp_job_get_perf_counter_value0(job, i);
			jobres->perf_counter1[i] = maliggy_pp_job_get_perf_counter_value1(job, i);
		}

		maliggy_session_send_notification(maliggy_pp_job_get_session(job), job->finished_notification);
	...
	if (MALI_TRUE == deferred)
	{
		...
		_maliggy_osk_lock_wait(pp_scheduler_job_delete_lock, _MALI_OSK_LOCKMODE_RW);

		MALI_DEBUG_ASSERT(_maliggy_osk_list_empty(&job->list)); /* This job object should not be on any list */
		_maliggy_osk_list_addtail(&job->list, &pp_scheduler_job_deletion_queue);

		_maliggy_osk_lock_signal(pp_scheduler_job_delete_lock, _MALI_OSK_LOCKMODE_RW);

		_maliggy_osk_wq_schedule_work(pp_scheduler_wq_job_delete);
	}
	else
	{
...
static void maliggy_pp_scheduler_do_job_delete(void *arg)
{
	 ...
	 */
	_maliggy_osk_list_move_list(&pp_scheduler_job_deletion_queue, &list);

	_maliggy_osk_lock_signal(pp_scheduler_job_delete_lock, _MALI_OSK_LOCKMODE_RW);

	_MALI_OSK_LIST_FOREACHENTRY(job, tmp, &list, struct maliggy_pp_job, list)
	{
		maliggy_pp_job_delete(job); /* delete the job object itself */
	}
}
#endif
...
{
	maliggy_bool job_is_done;
	maliggy_bool barrier_enforced = MALI_FALSE;

	MALI_DEBUG_PRINT(3, ("Mali PP scheduler: %s job %u (0x%08X) part %u/%u completed (%s)\n",
	                     maliggy_pp_job_is_virtual(job) ? "Virtual" : "Physical",
	                     maliggy_pp_job_get_id(job),
	                     job, sub_job + 1,
	                     maliggy_pp_job_get_sub_job_count(job),
	...
	maliggy_pp_job_mark_sub_job_completed(job, success);

	MALI_DEBUG_ASSERT(maliggy_pp_job_is_virtual(job) == maliggy_group_is_virtual(group));

	job_is_done = maliggy_pp_job_is_complete(job);

	if (job_is_done)
	{
		struct maliggy_session_data *session = maliggy_pp_job_get_session(job);
		struct maliggy_pp_job *job_head;

		/* Remove job from session list */
		_maliggy_osk_list_del(&job->session_list);

		MALI_DEBUG_PRINT(4, ("Mali PP scheduler: All parts completed for %s job %u (0x%08X)\n",
		                     maliggy_pp_job_is_virtual(job) ? "virtual" : "physical",
		                     maliggy_pp_job_get_id(job), job));
		...
		if (job->sync_point)
		{
			...
			MALI_DEBUG_PRINT(4, ("Sync: Signal %spoint for job %d\n",
			                     success ? "" : "failed ",
					     maliggy_pp_job_get_id(job)));
			maliggy_sync_signal_pt(job->sync_point, error);
		}
#endif

		/* Send notification back to user space */
#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
		maliggy_pp_scheduler_return_job_to_user(job, MALI_TRUE);
#else
		maliggy_pp_scheduler_return_job_to_user(job, MALI_FALSE);
#endif

		maliggy_pp_scheduler_job_completed();

		/* Resolve any barriers */
		if (!_maliggy_osk_list_empty(&session->job_list))
		{
			job_head = _MALI_OSK_LIST_ENTRY(session->job_list.next, struct maliggy_pp_job, session_list);
			if (maliggy_pp_job_has_active_barrier(job_head))
			{
				barrier_enforced = MALI_TRUE;
				maliggy_pp_job_barrier_enforced(job_head);
			}
		}
	}
	...
	if (pause_count > 0)
	{
		...
		{
			_maliggy_osk_list_move(&(group->pp_scheduler_list), &group_list_idle);
		}
#if defined(CONFIG_GPU_TRACEPOINTS) && defined(CONFIG_TRACEPOINTS)
			trace_gpu_sched_switch(maliggy_pp_get_hw_core_desc(group->pp_core), sched_clock(), 0, 0, 0);
#endif
		_maliggy_osk_wait_queue_wake_up(pp_scheduler_working_wait_queue);
		maliggy_pp_scheduler_unlock();
		return;
	}
	...
	if (barrier_enforced)
	{
		...
		_maliggy_osk_wq_schedule_work(pp_scheduler_wq_schedule);
	}
	...
	if (maliggy_group_is_virtual(group))
	{
		...
		struct maliggy_pp_job *physical_job = NULL;
		struct maliggy_group *physical_group = NULL;
		...
		if (maliggy_pp_scheduler_can_move_virtual_to_physical())
		{
			...
			physical_job = maliggy_pp_scheduler_get_physical_job();
			MALI_DEBUG_ASSERT(maliggy_pp_job_has_unstarted_sub_jobs(physical_job));

			/* Mark subjob as started */
			sub_job = maliggy_pp_job_get_first_unstarted_sub_job(physical_job);
			maliggy_pp_job_mark_sub_job_started(physical_job, sub_job);

			/* Remove job from queue (if we now got the last subjob) */
			maliggy_pp_scheduler_dequeue_physical_job(physical_job);
			...
			physical_group = maliggy_group_acquire_group(virtual_group);

			/* Move physical group to the working list, as we will soon start a job on it */
			_maliggy_osk_list_move(&(physical_group->pp_scheduler_list), &group_list_working);

			maliggy_pp_scheduler_disable_empty_virtual();
		}

		/* Start the next virtual job */
		job = maliggy_pp_scheduler_get_virtual_job();
		if (NULL != job && VIRTUAL_GROUP_IDLE == virtual_group_state)
		{
			...
			/* Start job */
			maliggy_group_start_pp_job(group, job, 0);
		...
		{
			maliggy_group_lock(physical_group);

			/* Set the group state from LEAVING_VIRTUAL to IDLE to complete the transition */
			physical_group->state = MALI_GROUP_STATE_IDLE;

			/* Start job on core */
			maliggy_group_start_pp_job(physical_group, physical_job, sub_job);
			MALI_DEBUG_PRINT(4, ("Mali PP scheduler: Physical job %u (0x%08X) part %u/%u started (from job_done)\n",
			                     maliggy_pp_job_get_id(physical_job), physical_job, sub_job + 1,
			                     maliggy_pp_job_get_sub_job_count(physical_job)));

			maliggy_group_unlock(physical_group);
		}
	}
	else
	{
		...
		if (NULL != job)
		{
			...
			/* Mark subjob as started */
			sub_job = maliggy_pp_job_get_first_unstarted_sub_job(job);
			maliggy_pp_job_mark_sub_job_started(job, sub_job);
		...
		else if (maliggy_pp_scheduler_has_virtual_group())
		{
			...
			_maliggy_osk_list_delinit(&(group->pp_scheduler_list));
			...
			if (maliggy_pp_scheduler_has_virtual_group())
			{
				maliggy_pp_scheduler_enable_empty_virtual();
			}
			...
			if (maliggy_pp_scheduler_has_virtual_group() && VIRTUAL_GROUP_IDLE == virtual_group_state)
			{
				_maliggy_osk_wq_schedule_work(pp_scheduler_wq_schedule);
			}
...
void maliggy_pp_scheduler_suspend(void)
{
	...
	/* Go to sleep. When woken up again (in maliggy_pp_scheduler_job_done), the
	 * maliggy_pp_scheduler_suspended() function will be called. This will return true
	 * iff state is idle and pause_count > 0, so if the core is active this
	 * will not do anything.
	 */
	_maliggy_osk_wait_queue_wait_event(pp_scheduler_working_wait_queue, maliggy_pp_scheduler_is_suspended);
}
...
MALI_STATIC_INLINE void maliggy_pp_scheduler_queue_job(struct maliggy_pp_job *job, struct maliggy_session_data *session)
{
	MALI_DEBUG_ASSERT_POINTER(job);

#if defined(CONFIG_GPU_TRACEPOINTS) && defined(CONFIG_TRACEPOINTS)
	trace_gpu_job_enqueue(maliggy_pp_job_get_tid(job), maliggy_pp_job_get_id(job), "PP");
#endif
	...
	{
		maliggy_dma_buf_map_job(job);
	}
#endif /* CONFIG_DMA_SHARED_BUFFER */

	maliggy_pp_scheduler_job_queued();
	...
	if (maliggy_pp_job_is_virtual(job))
	{
		...
		_maliggy_osk_list_addtail(&job->list, &virtual_job_queue);
	}
	else
	{
		job_queue_depth += maliggy_pp_job_get_sub_job_count(job);
		_maliggy_osk_list_addtail(&job->list, &job_queue);
	}

	if (maliggy_pp_job_has_active_barrier(job) && _maliggy_osk_list_empty(&session->job_list))
	{
		/* No running jobs on this session, so barrier condition already met */
		maliggy_pp_job_barrier_enforced(job);
	}

	/* Add job to session list */
	_maliggy_osk_list_addtail(&job->session_list, &session->job_list);

	MALI_DEBUG_PRINT(3, ("Mali PP scheduler: %s job %u (0x%08X) with %u parts queued\n",
	                     maliggy_pp_job_is_virtual(job) ? "Virtual" : "Physical",
	                     maliggy_pp_job_get_id(job), job, maliggy_pp_job_get_sub_job_count(job)));
...
{
	struct maliggy_pp_job *job = _MALI_OSK_CONTAINER_OF(waiter, struct maliggy_pp_job, sync_waiter);

	/* Schedule sync_callback_work */
	_maliggy_osk_wq_schedule_work(job->sync_work);
}

static void sync_callback_work(void *arg)
{
	struct maliggy_pp_job *job = (struct maliggy_pp_job *)arg;
	...
	if (likely(0 == err))
	{
		...
		maliggy_pp_scheduler_queue_job(job, session);
	...
	else
	{
		...
		if (job->sync_point) maliggy_sync_signal_pt(job->sync_point, err);

		maliggy_pp_job_mark_sub_job_completed(job, MALI_FALSE); /* Flagging the job as failed. */
		maliggy_pp_scheduler_return_job_to_user(job, MALI_FALSE); /* This will also delete the job object */
	}
}
#endif
...
_maliggy_osk_errcode_t _maliggy_ukk_pp_start_job(void *ctx, _maliggy_uk_pp_start_job_s *uargs, int *fence)
{
	...
	session = (struct maliggy_session_data*)ctx;

	job = maliggy_pp_job_create(session, uargs, maliggy_scheduler_get_new_id());
	if (NULL == job)
	{
	...
	if (_MALI_OSK_ERR_OK != maliggy_pp_job_check(job))
	{
...
#warning PP jobs will not be executed
	...
	if (_MALI_PP_JOB_FLAG_FENCE & job->uargs.flags)
	{
		...
		job->sync_point = maliggy_stream_create_point(job->uargs.stream);
		...
		post_fence = maliggy_stream_create_fence(job->sync_point);

		if (unlikely(0 > post_fence))
		{
			/* Fence creation failed. */
			/* maliggy_stream_create_fence already freed the sync_point */
	...
	else if (_MALI_PP_JOB_FLAG_EMPTY_FENCE & job->uargs.flags)
	{
		...
		ret = maliggy_sync_timed_commit(pt);
	...
	if (0 < job->uargs.fence)
	{
		...
		if (NULL == job->pre_fence)
		{
			...
			if (job->sync_point) maliggy_sync_signal_pt(job->sync_point, -EINVAL);
			maliggy_pp_job_mark_sub_job_completed(job, MALI_FALSE); /* Flagging the job as failed. */
			maliggy_pp_scheduler_return_job_to_user(job, MALI_FALSE); /* This will also delete the job object */
			return _MALI_OSK_ERR_OK; /* User is notified via a notification, so this call is ok */
		}

		job->sync_work = _maliggy_osk_wq_create_work(sync_callback_work, (void*)job);
		if (NULL == job->sync_work)
		{
			if (job->sync_point) maliggy_sync_signal_pt(job->sync_point, -ENOMEM);
			maliggy_pp_job_mark_sub_job_completed(job, MALI_FALSE); /* Flagging the job as failed. */
			maliggy_pp_scheduler_return_job_to_user(job, MALI_FALSE); /* This will also delete the job object */
			return _MALI_OSK_ERR_OK; /* User is notified via a notification, so this call is ok */
		}
		...
		_maliggy_osk_lock_wait(session->pending_jobs_lock, _MALI_OSK_LOCKMODE_RW);
		_maliggy_osk_list_addtail(&job->list, &session->pending_jobs);
		_maliggy_osk_lock_signal(session->pending_jobs_lock, _MALI_OSK_LOCKMODE_RW);
		...
		if (1 == err)
		{
			...
			maliggy_pp_scheduler_queue_job(job, session);
			if (!_maliggy_osk_list_empty(&group_list_idle) || VIRTUAL_GROUP_IDLE == virtual_group_state)
			{
				maliggy_pp_scheduler_schedule();
			}
			return _MALI_OSK_ERR_OK;
...
void _maliggy_ukk_pp_job_disable_wb(_maliggy_uk_pp_disable_wb_s *args)
{
	...
	session = (struct maliggy_session_data*)args->ctx;

	/* Check queue for jobs that match */
	maliggy_pp_scheduler_lock();
	_MALI_OSK_LIST_FOREACHENTRY(job, tmp, &job_queue, struct maliggy_pp_job, list)
	{
		if (maliggy_pp_job_get_session(job) == session &&
		    maliggy_pp_job_get_frame_builder_id(job) == (u32)args->fb_id &&
		    maliggy_pp_job_get_flush_id(job) == (u32)args->flush_id)
		{
			if (args->wbx & _MALI_UK_PP_JOB_WB0)
			{
				maliggy_pp_job_disable_wb0(job);
			}
			if (args->wbx & _MALI_UK_PP_JOB_WB1)
			{
				maliggy_pp_job_disable_wb1(job);
			}
			if (args->wbx & _MALI_UK_PP_JOB_WB2)
			{
				maliggy_pp_job_disable_wb2(job);
			}
			break;
		}
	}
...
{
	struct maliggy_pp_job *job, *tmp_job;
	struct maliggy_group *group, *tmp_group;
	struct maliggy_group *groups[MALI_MAX_NUMBER_OF_GROUPS];
	s32 i = 0;
#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
	...
	_MALI_OSK_LIST_FOREACHENTRY(job, tmp_job, &session->job_list, struct maliggy_pp_job, session_list)
	{
		...
		{
			MALI_DEBUG_ASSERT(1 == maliggy_pp_job_get_sub_job_count(job));
			if (0 == maliggy_pp_job_get_first_unstarted_sub_job(job))
			{
				--virtual_job_queue_depth;
			}
		}
		...
		{
			job_queue_depth -= maliggy_pp_job_get_sub_job_count(job) - maliggy_pp_job_get_first_unstarted_sub_job(job);
		}

		/* Mark all unstarted jobs as failed */
		maliggy_pp_job_mark_unstarted_failed(job);

		if (maliggy_pp_job_is_complete(job))
		{
			_maliggy_osk_list_del(&job->session_list);
...
#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
			MALI_DEBUG_ASSERT(_maliggy_osk_list_empty(&job->list));
			_maliggy_osk_list_addtail(&job->list, &deferred_deletion_list);
#else
			maliggy_pp_job_delete(job);
#endif
	...
	_MALI_OSK_LIST_FOREACHENTRY(group, tmp_group, &group_list_working, struct maliggy_group, pp_scheduler_list)
	{
		groups[i++] = group;
	}

	_MALI_OSK_LIST_FOREACHENTRY(group, tmp_group, &group_list_idle, struct maliggy_group, pp_scheduler_list)
	{
		groups[i++] = group;
	}
...
#if defined(MALI_PP_SCHEDULER_USE_DEFERRED_JOB_DELETE)
	_MALI_OSK_LIST_FOREACHENTRY(job, tmp_job, &deferred_deletion_list, struct maliggy_pp_job, list)
	{
		maliggy_pp_job_delete(job);
	}
#endif
	...
	while (i > 0)
	{
		maliggy_group_abort_session(groups[--i], session);
	}

	if (maliggy_pp_scheduler_has_virtual_group())
	{
		maliggy_group_abort_session(virtual_group, session);
	}
}
...
u32 maliggy_pp_scheduler_dumpggy_state(char *buf, u32 size)
{
	...
	n += _maliggy_osk_snprintf(buf + n, size - n, "PP:\n");
	n += _maliggy_osk_snprintf(buf + n, size - n, "\tQueue is %s\n", _maliggy_osk_list_empty(&job_queue) ? "empty" : "not empty");
	n += _maliggy_osk_snprintf(buf + n, size - n, "\n");

	_MALI_OSK_LIST_FOREACHENTRY(group, temp, &group_list_working, struct maliggy_group, pp_scheduler_list)
	{
		n += maliggy_group_dumpggy_state(group, buf + n, size - n);
	}
	...
	if (maliggy_pp_scheduler_has_virtual_group())
	{
		n += maliggy_group_dumpggy_state(virtual_group, buf + n, size -n);
	}
...
void maliggy_pp_scheduler_reset_all_groups(void)
{
	...
	{
		maliggy_group_lock(virtual_group);
		maliggy_group_reset(virtual_group);
		maliggy_group_unlock(virtual_group);
	}
...
void maliggy_pp_scheduler_zap_all_active(struct maliggy_session_data *session)
{
	...
	if (maliggy_pp_scheduler_has_virtual_group())
	{
		maliggy_group_zap_session(virtual_group, session);
	}
	...
	while (i > 0)
	{
		maliggy_group_zap_session(groups[--i], session);
	}
}

/* A pm reference must be taken with _maliggy_osk_pm_dev_ref_add_no_power_on
 * before calling this function to avoid Mali powering down as HW is accessed.
 */
static void maliggy_pp_scheduler_enable_group_internal(struct maliggy_group *group)
{
	MALI_DEBUG_ASSERT_POINTER(group);
	...
	if (maliggy_pp_scheduler_has_virtual_group())
	{
		...
		update_hw = maliggy_pm_is_power_on();

		maliggy_pm_domain_ref_get(group->pm_domain);
		MALI_DEBUG_ASSERT(NULL == group->pm_domain ||
		                  MALI_PM_DOMAIN_ON == maliggy_pm_domain_state_get(group->pm_domain));
		...
		maliggy_pp_scheduler_enable_empty_virtual();
		maliggy_group_add_group(virtual_group, group, update_hw);
...
void maliggy_pp_scheduler_enable_group(struct maliggy_group *group)
{
	...
	maliggy_pp_scheduler_enable_group_internal(group);
...
static void maliggy_pp_scheduler_disable_group_internal(struct maliggy_group *group)
{
	if (maliggy_pp_scheduler_has_virtual_group())
	{
		maliggy_group_lock(virtual_group);
		...
		else if (MALI_GROUP_STATE_IN_VIRTUAL == group->state)
		{
			...
			maliggy_group_remove_group(virtual_group, group);
	...
	else
	{
		...
		--enabled_cores;
		_maliggy_osk_list_move(&(group->pp_scheduler_list), &group_list_disabled);
		group->state = MALI_GROUP_STATE_DISABLED;

		maliggy_group_power_off_group(group);
		maliggy_pm_domain_ref_put(group->pm_domain);
	}
...
void maliggy_pp_scheduler_disable_group(struct maliggy_group *group)
{
	...
	maliggy_pp_scheduler_disable_group_internal(group);
...
static void maliggy_pp_scheduler_notify_core_change(u32 num_cores)
{
	if (!maliggy_is_maliggy450())
	{
		...
		{
			_maliggy_osk_notification_t *notobj = _maliggy_osk_notification_create(_MALI_NOTIFICATION_PP_NUM_CORE_CHANGE, sizeof(_maliggy_uk_pp_num_cores_changed_s));
			if (NULL != notobj)
			{
				_maliggy_uk_pp_num_cores_changed_s *data = notobj->result_buffer;
...
static void maliggy_pp_scheduler_set_perf_level_maliggy400(u32 target_core_nr)
{
	struct maliggy_group *group;
	...
	if (target_core_nr > enabled_cores)
	{
		...
		_maliggy_osk_pm_dev_ref_add_no_power_on();
		_maliggy_osk_pm_dev_barrier();

		while (target_core_nr > enabled_cores)
		{
			maliggy_pp_scheduler_lock();

			MALI_DEBUG_ASSERT(!_maliggy_osk_list_empty(&group_list_disabled));

			group = _MALI_OSK_LIST_ENTRY(group_list_disabled.next, struct maliggy_group, pp_scheduler_list);
	...
	else if (target_core_nr < enabled_cores)
	{
		...
		while (target_core_nr < enabled_cores)
		{
			...
			MALI_DEBUG_ASSERT(!_maliggy_osk_list_empty(&group_list_idle));
			MALI_DEBUG_ASSERT(_maliggy_osk_list_empty(&group_list_working));

			group = _MALI_OSK_LIST_ENTRY(group_list_idle.next, struct maliggy_group, pp_scheduler_list);
	...
	maliggy_pp_scheduler_notify_core_change(target_core_nr);
}

static void maliggy_pp_scheduler_set_perf_level_maliggy450(u32 target_core_nr)
{
	struct maliggy_group *group;
	MALI_DEBUG_ASSERT(maliggy_is_maliggy450());
	...
	if (target_core_nr > enabled_cores)
	{
		...
		domain = maliggy_pm_domain_get(MALI_PMU_M450_DOM2);
		...
		if (target_core_nr > enabled_cores)
		{
			domain = maliggy_pm_domain_get(MALI_PMU_M450_DOM3);
			MALI_PM_DOMAIN_FOR_EACH_GROUP(group, domain)
			{
				maliggy_pp_scheduler_enable_group_internal(group);
				if (target_core_nr == enabled_cores) break;
...
int maliggy_pp_scheduler_set_perf_level(unsigned int cores)
{
	...
	{
		/* Mali-400 */
		maliggy_pp_scheduler_set_perf_level_maliggy400(cores);
	}
	else
	{
		/* Mali-450 */
		maliggy_pp_scheduler_set_perf_level_maliggy450(cores);
	}
...
static void maliggy_pp_scheduler_job_queued(void)
{
	...
	if (maliggy_utilization_enabled())
	{
		/*
		 ...
		 */
		maliggy_utilization_pp_start();
	}
}

static void maliggy_pp_scheduler_job_completed(void)
{
	/* Release the PM reference we got in the maliggy_pp_scheduler_job_queued() function */
	_maliggy_osk_pm_dev_ref_dec();

	if (maliggy_utilization_enabled())
	{
		maliggy_utilization_pp_end();
	}
}
