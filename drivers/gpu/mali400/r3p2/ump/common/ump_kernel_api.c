...
UMP_KERNEL_API_EXPORT umpggy_secure_id umpggy_dd_secure_id_get(umpggy_dd_handle memh)
{
	umpggy_dd_mem * mem = (umpggy_dd_mem *)memh;
...
UMP_KERNEL_API_EXPORT unsigned long umpggy_dd_phys_block_count_get(umpggy_dd_handle memh)
{
	umpggy_dd_mem * mem = (umpggy_dd_mem*) memh;
...
UMP_KERNEL_API_EXPORT umpggy_dd_status_code umpggy_dd_phys_blocks_get(umpggy_dd_handle memh, umpggy_dd_physical_block * blocks, unsigned long num_blocks)
{
	umpggy_dd_mem * mem = (umpggy_dd_mem *)memh;
	...
	if (blocks == NULL)
	{
		DBG_MSG(1, ("NULL parameter in umpggy_dd_phys_blocks_get()\n"));
	...
	_maliggy_osk_memcpy(blocks, mem->block_array, sizeof(umpggy_dd_physical_block) * mem->nr_blocks);
...
UMP_KERNEL_API_EXPORT umpggy_dd_status_code umpggy_dd_phys_block_get(umpggy_dd_handle memh, unsigned long index, umpggy_dd_physical_block * block)
{
	umpggy_dd_mem * mem = (umpggy_dd_mem *)memh;
	...
	if (block == NULL)
	{
		DBG_MSG(1, ("NULL parameter in umpggy_dd_phys_block_get()\n"));
		return UMP_DD_INVALID;
	}

	if (index >= mem->nr_blocks)
	{
		DBG_MSG(5, ("Invalid index specified in umpggy_dd_phys_block_get()\n"));
		return UMP_DD_INVALID;
	}
...
UMP_KERNEL_API_EXPORT unsigned long umpggy_dd_size_get(umpggy_dd_handle memh)
{
	umpggy_dd_mem * mem = (umpggy_dd_mem*)memh;
...
UMP_KERNEL_API_EXPORT void umpggy_dd_reference_add(umpggy_dd_handle memh)
{
	umpggy_dd_mem * mem = (umpggy_dd_mem*)memh;
	int new_ref;
	...
	new_ref = _umpggy_osk_atomic_inc_and_read(&mem->ref_count);
...
UMP_KERNEL_API_EXPORT void umpggy_dd_reference_release(umpggy_dd_handle memh)
{
	int new_ref;
	umpggy_dd_mem * mem = (umpggy_dd_mem*)memh;
	...
	that elements in the umpggy_descriptor_mapping table is always valid.  If they
	...
	new_ref = _umpggy_osk_atomic_dec_and_read(&mem->ref_count);
	...
	if (0 == new_ref)
	{
		...
		umpggy_descriptor_mapping_free(device.secure_id_map, (int)mem->secure_id);
...
{
	umpggy_session_data * session_data;
...
{
	umpggy_session_memory_list_element * session_memory_element;
	umpggy_session_memory_list_element * tmp;
	umpggy_session_data * session_data;
	_maliggy_osk_errcode_t ret = _MALI_OSK_ERR_INVALID_FUNC;
	int secure_id;
	...
	session_data = (umpggy_session_data*)release_info->ctx;
	...
	_MALI_OSK_LIST_FOREACHENTRY(session_memory_element, tmp, &session_data->list_head_session_memory_list, umpggy_session_memory_list_element, list)
	{
		if ( session_memory_element->mem->secure_id == secure_id)
		{
			umpggy_dd_mem *release_mem;

			release_mem = session_memory_element->mem;
			_maliggy_osk_list_del(&session_memory_element->list);
			umpggy_dd_reference_release(release_mem);
	...
	DBG_MSG(4, ("_umpggy_ukk_release() returning 0x%x\n", ret));
	return ret;
}
...
{
	umpggy_dd_mem * mem;
	_maliggy_osk_errcode_t ret = _MALI_OSK_ERR_FAULT;

	DEBUG_ASSERT_POINTER( user_interaction );

	/* We lock the mappings so things don't get removed while we are looking for the memory */
	_maliggy_osk_lock_wait(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
	if (0 == umpggy_descriptor_mapping_get(device.secure_id_map, (int)user_interaction->secure_id, (void**)&mem))
	{
		user_interaction->size = mem->size_bytes;
		DBG_MSG(4, ("Returning size. ID: %u, size: %lu ", (umpggy_secure_id)user_interaction->secure_id, (unsigned long)user_interaction->size));
		ret = _MALI_OSK_ERR_OK;
	}
	else
	{
		 user_interaction->size = 0;
		DBG_MSG(1, ("Failed to look up mapping in umpggy_ioctl_size_get(). ID: %u\n", (umpggy_secure_id)user_interaction->secure_id));
	}
...
{
	umpggy_dd_mem * mem = NULL;
	void *virtual = NULL;
	u32 size = 0;
	u32 offset = 0;

	_maliggy_osk_lock_wait(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
	umpggy_descriptor_mapping_get(device.secure_id_map, (int)args->secure_id, (void**)&mem);

	if (NULL == mem)
	{
		_maliggy_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
		DBG_MSG(1, ("Failed to look up mapping in _umpggy_ukk_msync(). ID: %u\n", (umpggy_secure_id)args->secure_id));
		return;
	}
	...
	{
		DBG_MSG(3, ("_umpggy_ukk_msync READOUT  ID: %u Enabled: %d\n", (umpggy_secure_id)args->secure_id, mem->is_cached));
	...
	if ( 0==mem->is_cached )
	{
		DBG_MSG(3, ("_umpggy_ukk_msync IGNORING ID: %u Enabled: %d  OP: %d\n", (umpggy_secure_id)args->secure_id, mem->is_cached, args->op));
		goto msync_release_and_return;
	}
	DBG_MSG(3, ("UMP[%02u] _umpggy_ukk_msync  Flush  OP: %d Address: 0x%08x Mapping: 0x%08x\n",
	            (umpggy_secure_id)args->secure_id, args->op, args->address, args->mapping));
	...
	_umpggy_osk_msync( mem, virtual, offset, size, args->op, NULL);
...
void _umpggy_ukk_cache_operations_control(_umpggy_uk_cache_operations_control_s* args)
{
	...
	else if ( op== _UMP_UK_CACHE_OP_FINISH )
	{
		...
		if ( session_data->has_pending_level1_cache_flush)
		{
			...
			_umpggy_osk_msync( NULL, NULL, 0, 0, _UMP_UK_MSYNC_FLUSH_L1, session_data);
		}
		#endif
...
{
	umpggy_dd_mem * mem = NULL;
	umpggy_uk_user old_user;
	umpggy_uk_msync_op cache_op = _UMP_UK_MSYNC_CLEAN_AND_INVALIDATE;
	umpggy_session_data *session_data;
	...
	{
		_maliggy_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
		DBG_MSG(1, ("Failed to look up mapping in _umpggy_ukk_switch_hw_usage(). ID: %u\n", (umpggy_secure_id)args->secure_id));
		return;
	}
	...
	DBG_MSG(3, ("UMP[%02u] Switch usage  Start  New: %s  Prev: %s.\n", (umpggy_secure_id)args->secure_id, args->new_user?"MALI":"CPU",old_user?"MALI":"CPU"));
...
#error
	...
	_maliggy_osk_lock_wait(session_data->lock, _MALI_OSK_LOCKMODE_RW);
	/* Actual cache flush */
	_umpggy_osk_msync( mem, NULL, 0, mem->size_bytes, cache_op, session_data);
	_maliggy_osk_lock_signal(session_data->lock, _MALI_OSK_LOCKMODE_RW);
...
void _umpggy_ukk_lock(_umpggy_uk_lock_s *args )
{
	...
	{
		_maliggy_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
		DBG_MSG(1, ("UMP[%02u] Failed to look up mapping in _umpggy_ukk_lock(). ID: %u\n", (umpggy_secure_id)args->secure_id));
		return;
	}
	umpggy_dd_reference_add(mem);
	_maliggy_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
	...
	mem->lock_usage = (umpggy_lock_usage) args->lock_usage;
...
void _umpggy_ukk_unlock(_umpggy_uk_unlock_s *args )
{
	...
	{
		_maliggy_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
		DBG_MSG(1, ("Failed to look up mapping in _umpggy_ukk_unlock(). ID: %u\n", (umpggy_secure_id)args->secure_id));
		return;
	}
	umpggy_dd_reference_add(mem);
	_maliggy_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
	...
	mem->lock_usage = (umpggy_lock_usage) UMP_NOT_LOCKED;
