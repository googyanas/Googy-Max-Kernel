...
/* types used for external_memory and umpggy_memory physical memory allocators, which are using the maliggy_allocation_engine */
#if defined(CONFIG_MALI400_UMP)
typedef struct umpggy_mem_allocation
{
	maliggy_allocation_engine * engine;
	maliggy_memory_allocation * descriptor;
	u32 initial_offset;
	u32 size_allocated;
	umpggy_dd_handle umpggy_mem;
} umpggy_mem_allocation ;
#endif
 ...
 * wrapper _maliggy_ukk_mem_munmap() will lock the descriptor, then call this
 * function to do the actual unmapping. maliggy_memory_core_session_end() could
 ...
 */
static _maliggy_osk_errcode_t _maliggy_ukk_mem_munmap_internal( _maliggy_uk_mem_munmap_s *args );

#if defined(CONFIG_MALI400_UMP)
static void umpggy_memory_release(void * ctx, void * handle);
static maliggy_physical_memory_allocation_result umpggy_memory_commit(void* ctx, maliggy_allocation_engine * engine, maliggy_memory_allocation * descriptor, u32* offset, maliggy_physical_memory_allocation * alloc_info);
#endif /* CONFIG_MALI400_UMP */


static void external_memory_release(void * ctx, void * handle);
static maliggy_physical_memory_allocation_result external_memory_commit(void* ctx, maliggy_allocation_engine * engine, maliggy_memory_allocation * descriptor, u32* offset, maliggy_physical_memory_allocation * alloc_info);
...
static _maliggy_osk_errcode_t  maliggy_address_manager_allocate(maliggy_memory_allocation * descriptor); /* validates the range, allocates memory for the page tables if needed */
static _maliggy_osk_errcode_t  maliggy_address_manager_map(maliggy_memory_allocation * descriptor, u32 offset, u32 *phys_addr, u32 size);
static void maliggy_address_manager_release(maliggy_memory_allocation * descriptor);

/* MMU variables */

typedef struct maliggy_mmu_page_table_allocation
{
	_maliggy_osk_list_t list;
	u32 * usage_map;
	u32 usage_count;
	u32 num_pages;
	maliggy_page_table_block pages;
} maliggy_mmu_page_table_allocation;

typedef struct maliggy_mmu_page_table_allocations
{
	_maliggy_osk_lock_t *lock;
	_maliggy_osk_list_t partial;
	_maliggy_osk_list_t full;
	/* we never hold on to a empty allocation */
} maliggy_mmu_page_table_allocations;

static maliggy_kernel_mem_address_manager maliggy_address_manager =
{
	maliggy_address_manager_allocate, /* allocate */
	maliggy_address_manager_release,  /* release */
	maliggy_address_manager_map,      /* map_physical */
	NULL                           /* unmap_physical not present*/
};

/* the mmu page table cache */
static struct maliggy_mmu_page_table_allocations page_table_cache;


static maliggy_kernel_mem_address_manager process_address_manager =
{
	_maliggy_osk_mem_mapregion_init,  /* allocate */
	_maliggy_osk_mem_mapregion_term,  /* release */
	_maliggy_osk_mem_mapregion_map,   /* map_physical */
	_maliggy_osk_mem_mapregion_unmap  /* unmap_physical */
};

static _maliggy_osk_errcode_t maliggy_mmu_page_table_cache_create(void);
static void maliggy_mmu_page_table_cache_destroy(void);

static maliggy_allocation_engine memory_engine = NULL;
static maliggy_physical_memory_allocator * physical_memory_allocators = NULL;
...
_maliggy_osk_errcode_t maliggy_memory_initialize(void)
{
	...
	err = maliggy_mmu_page_table_cache_create();
	if(_MALI_OSK_ERR_OK != err)
	{
		MALI_ERROR(err);
	}

	memory_engine = maliggy_allocation_engine_create(&maliggy_address_manager, &process_address_manager);
...
void maliggy_memory_terminate(void)
{
	...
	maliggy_mmu_page_table_cache_destroy();
	...
	while ( NULL != mem_region_registrations)
	{
		...
		_maliggy_osk_mem_unreqregion(m->base, m->size);
		_maliggy_osk_free(m);
	}

	while ( NULL != physical_memory_allocators)
	{
		maliggy_physical_memory_allocator * m;
	...
	{
		maliggy_allocation_engine_destroy(memory_engine);
		memory_engine = NULL;
	}
}

_maliggy_osk_errcode_t maliggy_memory_session_begin(struct maliggy_session_data * session_data)
{
	MALI_DEBUG_PRINT(5, ("Memory session begin\n"));

	/* create descriptor mapping table */
	session_data->descriptor_mapping = maliggy_descriptor_mapping_create(MALI_MEM_DESCRIPTORS_INIT, MALI_MEM_DESCRIPTORS_MAX);
	...
	session_data->memory_lock = _maliggy_osk_lock_init( _MALI_OSK_LOCKFLAG_ORDERED | _MALI_OSK_LOCKFLAG_ONELOCK
	                                | _MALI_OSK_LOCKFLAG_NONINTERRUPTABLE, 0, _MALI_OSK_LOCK_ORDER_MEM_SESSION);
	if (NULL == session_data->memory_lock)
	{
		maliggy_descriptor_mapping_destroy(session_data->descriptor_mapping);
...
static void descriptor_table_cleanup_callback(int descriptor_id, void* map_target)
{
	...
	descriptor = (maliggy_memory_allocation*)map_target;

	MALI_DEBUG_PRINT(3, ("Cleanup of descriptor %d mapping to 0x%x in descriptor table\n", descriptor_id, map_target));
	MALI_DEBUG_ASSERT(descriptor);

	maliggy_allocation_engine_release_memory(memory_engine, descriptor);
	_maliggy_osk_free(descriptor);
}

void maliggy_memory_session_end(struct maliggy_session_data *session_data)
{
	_maliggy_osk_errcode_t err = _MALI_OSK_ERR_BUSY;
	...
	while (err == _MALI_OSK_ERR_BUSY)
	{
		...
		_maliggy_osk_lock_wait( session_data->memory_lock, _MALI_OSK_LOCKMODE_RW );
		err = _MALI_OSK_ERR_OK;

		/* Free all memory engine allocations */
		if (!_maliggy_osk_list_empty(&session_data->memory_head))
		{
			maliggy_memory_allocation *descriptor;
			maliggy_memory_allocation *temp;
			_maliggy_uk_mem_munmap_s unmap_args;
			...
			_MALI_OSK_LIST_FOREACHENTRY(descriptor, temp, &session_data->memory_head, maliggy_memory_allocation, list)
			{
				MALI_DEBUG_PRINT(4, ("Freeing block with mali address 0x%x size %d mapped in user space at 0x%x\n",
							descriptor->maliggy_address, descriptor->size, descriptor->size, descriptor->mapping)
						);
				...
				/*
					...
					*/
				err = _maliggy_ukk_mem_munmap_internal( &unmap_args );

				if (err == _MALI_OSK_ERR_BUSY)
				{
					_maliggy_osk_lock_signal( session_data->memory_lock, _MALI_OSK_LOCKMODE_RW );
					/*
						* Reason for this;
						...
						*/
					_maliggy_osk_time_ubusydelay(10);
	...
	MALI_DEBUG_ASSERT( _maliggy_osk_list_empty(&session_data->memory_head) );

	if (NULL != session_data->descriptor_mapping)
	{
		maliggy_descriptor_mapping_call_for_each(session_data->descriptor_mapping, descriptor_table_cleanup_callback);
		maliggy_descriptor_mapping_destroy(session_data->descriptor_mapping);
	...
	/* Free the lock */
	_maliggy_osk_lock_term( session_data->memory_lock );
...
_maliggy_osk_errcode_t maliggy_memory_core_resource_os_memory(u32 size)
{
	...
	maliggy_physical_memory_allocator ** next_allocator_list;

	u32 alloc_order = 1; /* OS memory has second priority */

	allocator = maliggy_os_allocator_create(size, 0 /* cpu_usage_adjust */, "Shared Mali GPU memory");
	if (NULL == allocator)
	{
...
_maliggy_osk_errcode_t maliggy_memory_core_resource_dedicated_memory(u32 start, u32 size)
{
	...
	if (_MALI_OSK_ERR_OK != _maliggy_osk_mem_reqregion(start, size, "Dedicated Mali GPU memory"))
	{
	...
	allocator = maliggy_block_allocator_create(start, 0 /* cpu_usage_adjust */, size, "Dedicated Mali GPU memory");

	if (NULL == allocator)
	{
		MALI_DEBUG_PRINT(1, ("Memory bank registration failed\n"));
		_maliggy_osk_mem_unreqregion(start, size);
		MALI_ERROR(_MALI_OSK_ERR_FAULT);
	}
	...
	cleanup_data = _maliggy_osk_malloc(sizeof(dedicated_memory_info));
...
static maliggy_physical_memory_allocation_result umpggy_memory_commit(void* ctx, maliggy_allocation_engine * engine, maliggy_memory_allocation * descriptor, u32* offset, maliggy_physical_memory_allocation * alloc_info)
{
	...
	u32 nr_blocks;
	u32 i;
	umpggy_dd_physical_block * umpggy_blocks;
	umpggy_mem_allocation *ret_allocation;
	...
	ret_allocation = _maliggy_osk_malloc( sizeof( umpggy_mem_allocation ) );
	if ( NULL==ret_allocation ) return MALI_MEM_ALLOC_INTERNAL_FAILURE;

	umpggy_mem = (umpggy_dd_handle)ctx;

	MALI_DEBUG_PRINT(4, ("In umpggy_memory_commit\n"));

	nr_blocks = umpggy_dd_phys_block_count_get(umpggy_mem);
	...
	umpggy_blocks = _maliggy_osk_malloc(sizeof(*umpggy_blocks)*nr_blocks );
	if ( NULL==umpggy_blocks )
	{
		_maliggy_osk_free( ret_allocation );
		return MALI_MEM_ALLOC_INTERNAL_FAILURE;
	}

	if (UMP_DD_INVALID == umpggy_dd_phys_blocks_get(umpggy_mem, umpggy_blocks, nr_blocks))
	{
		_maliggy_osk_free(umpggy_blocks);
		_maliggy_osk_free( ret_allocation );
		return MALI_MEM_ALLOC_INTERNAL_FAILURE;
	}
	...
	{
		MALI_DEBUG_PRINT(4, ("Mapping in 0x%08x size %d\n", umpggy_blocks[i].addr , umpggy_blocks[i].size));
		if (_MALI_OSK_ERR_OK != maliggy_allocation_engine_map_physical(engine, descriptor, *offset, umpggy_blocks[i].addr , 0, umpggy_blocks[i].size ))
		{
			...
			maliggy_allocation_engine_unmap_physical(engine, descriptor, ret_allocation->initial_offset, size_allocated, (_maliggy_osk_mem_mapregion_flags_t)0 );

			_maliggy_osk_free(umpggy_blocks);
			_maliggy_osk_free(ret_allocation);
			return MALI_MEM_ALLOC_INTERNAL_FAILURE;
		}
		*offset += umpggy_blocks[i].size;
	}
	...
	if (descriptor->flags & MALI_MEMORY_ALLOCATION_FLAG_MAP_GUARD_PAGE)
	{
		...
		if (_MALI_OSK_ERR_OK != maliggy_allocation_engine_map_physical(engine, descriptor, *offset, umpggy_blocks[0].addr , 0, _MALI_OSK_MALI_PAGE_SIZE ))
		{
			u32 size_allocated = *offset - ret_allocation->initial_offset;
...
{
	umpggy_dd_handle umpggy_mem;
	umpggy_mem_allocation *allocation;

	allocation = (umpggy_mem_allocation *)handle;

	MALI_DEBUG_ASSERT_POINTER( allocation );

	umpggy_mem = allocation->umpggy_mem;

	MALI_DEBUG_ASSERT(UMP_DD_HANDLE_INVALID!=umpggy_mem);

	/* At present, this is a no-op. But, it allows the maliggy_address_manager to
	 * do unmapping of a subrange in future. */
	maliggy_allocation_engine_unmap_physical( allocation->engine,
										   allocation->descriptor,
										   allocation->initial_offset,
										   allocation->size_allocated,
										   (_maliggy_osk_mem_mapregion_flags_t)0
										   );
	_maliggy_osk_free( allocation );


	umpggy_dd_reference_release(umpggy_mem) ;
	return;
}
...
_maliggy_osk_errcode_t _maliggy_ukk_attach_umpggy_mem( _maliggy_uk_attach_umpggy_mem_s *args )
{
	...
	MALI_DEBUG_PRINT(3,
	                 ("Requested to map ump memory with secure id %d into virtual memory 0x%08X, size 0x%08X\n",
	                  args->secure_id, args->maliggy_address, args->size));

	umpggy_mem = umpggy_dd_handle_create_from_secure_id( (int)args->secure_id ) ;

	if ( UMP_DD_HANDLE_INVALID==umpggy_mem ) MALI_ERROR(_MALI_OSK_ERR_FAULT);
	...
	descriptor->maliggy_addr_mapping_info = (void*)session_data;
	...
	if (_MALI_OSK_ERR_OK != maliggy_descriptor_mapping_allocate_mapping(session_data->descriptor_mapping, descriptor, &md))
	{
		umpggy_dd_reference_release(umpggy_mem);
		_maliggy_osk_free(descriptor);
		MALI_ERROR(_MALI_OSK_ERR_FAULT);
	...
	external_memory_allocator.allocate = umpggy_memory_commit;
	external_memory_allocator.allocate_page_table_block = NULL;
	external_memory_allocator.ctx = umpggy_mem;
	external_memory_allocator.name = "UMP Memory";
	external_memory_allocator.next = NULL;

	_maliggy_osk_lock_wait(session_data->memory_lock, _MALI_OSK_LOCKMODE_RW);

	if (_MALI_OSK_ERR_OK != maliggy_allocation_engine_allocate_memory(memory_engine, descriptor, &external_memory_allocator, NULL))
	{
		_maliggy_osk_lock_signal(session_data->memory_lock, _MALI_OSK_LOCKMODE_RW);
		maliggy_descriptor_mapping_free(session_data->descriptor_mapping, md);
		umpggy_dd_reference_release(umpggy_mem);
		_maliggy_osk_free(descriptor);
		MALI_ERROR(_MALI_OSK_ERR_NOMEM);
	}
...
_maliggy_osk_errcode_t _maliggy_ukk_release_umpggy_mem( _maliggy_uk_release_umpggy_mem_s *args )
{
	...
	if (_MALI_OSK_ERR_OK != maliggy_descriptor_mapping_get(session_data->descriptor_mapping, args->cookie, (void**)&descriptor))
	{
		MALI_DEBUG_PRINT(1, ("Invalid memory descriptor %d used to release ump memory\n", args->cookie));
		MALI_ERROR(_MALI_OSK_ERR_FAULT);
	}

	descriptor = maliggy_descriptor_mapping_free(session_data->descriptor_mapping, args->cookie);
...
static maliggy_physical_memory_allocation_result external_memory_commit(void* ctx, maliggy_allocation_engine * engine, maliggy_memory_allocation * descriptor, u32* offset, maliggy_physical_memory_allocation * alloc_info)
{
	...
	ret_allocation = _maliggy_osk_malloc( sizeof(external_mem_allocation) );
	...
	MALI_DEBUG_PRINT(5, ("External map: mapping phys 0x%08X at mali virtual address 0x%08X staring at offset 0x%08X length 0x%08X\n", data[0], descriptor->maliggy_address, *offset, data[1]));

	if (_MALI_OSK_ERR_OK != maliggy_allocation_engine_map_physical(engine, descriptor, *offset, data[0], 0, data[1]))
	{
		MALI_DEBUG_PRINT(1, ("Mapping of external memory failed\n"));
		_maliggy_osk_free(ret_allocation);
		return MALI_MEM_ALLOC_INTERNAL_FAILURE;
	...
	*offset += data[1];
	...
	if (descriptor->flags & MALI_MEMORY_ALLOCATION_FLAG_MAP_GUARD_PAGE)
	{
		/* Map in an extra virtual guard page at the end of the VMA */
		MALI_DEBUG_PRINT(4, ("Mapping in extra guard page\n"));
		if (_MALI_OSK_ERR_OK != maliggy_allocation_engine_map_physical(engine, descriptor, *offset, data[0], 0, _MALI_OSK_MALI_PAGE_SIZE))
		{
			u32 size_allocated = *offset - ret_allocation->initial_offset;
			MALI_DEBUG_PRINT(1, ("Mapping of external memory (guard page) failed\n"));
...
_maliggy_osk_errcode_t _maliggy_ukk_mem_write_safe(_maliggy_uk_mem_write_safe_s *args)
{
	...
	args->size = _maliggy_osk_mem_write_safe(args->dest, args->src, args->size);
...
_maliggy_osk_errcode_t _maliggy_ukk_map_external_mem( _maliggy_uk_map_external_mem_s *args )
{
	...
	MALI_DEBUG_PRINT(3,
	        ...
	        (void*)args->maliggy_address)
	);

	/* Validate the mali physical range */
	if (_MALI_OSK_ERR_OK != maliggy_mem_validation_check(args->phys_addr, args->size))
	{
...
_maliggy_osk_errcode_t _maliggy_ukk_unmap_external_mem( _maliggy_uk_unmap_external_mem_s *args )
{
	...
	old_value = maliggy_descriptor_mapping_free(session_data->descriptor_mapping, args->cookie);
...
_maliggy_osk_errcode_t _maliggy_ukk_init_mem( _maliggy_uk_init_mem_s *args )
{
	...
	args->maliggy_address_base = 1 * 1024 * 1024 * 1024UL; /* staring at 1GB, causing this layout: (0-1GB unused)(1GB-3G usage by Mali)(3G-4G unused) */
	MALI_SUCCESS;
}
...
static _maliggy_osk_errcode_t maliggy_address_manager_allocate(maliggy_memory_allocation * descriptor)
{
	struct maliggy_session_data *session_data;
	u32 actual_size;
	...
	session_data = (struct maliggy_session_data *)descriptor->maliggy_addr_mapping_info;
	...
	return maliggy_mmu_pagedir_map(session_data->page_directory, descriptor->maliggy_address, actual_size);
}

static void maliggy_address_manager_release(maliggy_memory_allocation * descriptor)
{
	const u32 illegal_maliggy_address = 0xffffffff;
	struct maliggy_session_data *session_data;
	MALI_DEBUG_ASSERT_POINTER(descriptor);

	/* It is allowed to call this function several times on the same descriptor.
	   When memory is released we set the illegal_maliggy_address so we can early out here. */
	if ( illegal_maliggy_address == descriptor->maliggy_address) return;

	session_data = (struct maliggy_session_data *)descriptor->maliggy_addr_mapping_info;
	maliggy_mmu_pagedir_unmap(session_data->page_directory, descriptor->maliggy_address, descriptor->size);

	descriptor->maliggy_address = illegal_maliggy_address ;
}
...
static _maliggy_osk_errcode_t maliggy_address_manager_map(maliggy_memory_allocation * descriptor, u32 offset, u32 *phys_addr, u32 size)
{
	...
	maliggy_address = descriptor->maliggy_address + offset;

	MALI_DEBUG_PRINT(7, ("Mali map: mapping 0x%08X to Mali address 0x%08X length 0x%08X\n", *phys_addr, maliggy_address, size));

	maliggy_mmu_pagedir_update(session_data->page_directory, maliggy_address, *phys_addr, size, descriptor->cache_settings);

	MALI_SUCCESS;
}

/* This handler registered to maliggy_mmap for MMU builds */
_maliggy_osk_errcode_t _maliggy_ukk_mem_mmap( _maliggy_uk_mem_mmap_s *args )
{
	...
	descriptor = (maliggy_memory_allocation*) _maliggy_osk_calloc( 1, sizeof(maliggy_memory_allocation) );
	if (NULL == descriptor) { MALI_DEBUG_PRINT(3,("mali_ukk_mem_mmap: descriptor was NULL\n")); MALI_ERROR(_MALI_OSK_ERR_NOMEM); }

	descriptor->size = args->size;
	descriptor->maliggy_address = args->phys_addr;
	descriptor->maliggy_addr_mapping_info = (void*)session_data;
	...
	if (0 == maliggy_allocation_engine_allocate_memory(memory_engine, descriptor, physical_memory_allocators, &session_data->memory_head))
	{
...
static _maliggy_osk_errcode_t _maliggy_ukk_mem_munmap_internal( _maliggy_uk_mem_munmap_s *args )
{
	...
	descriptor = (maliggy_memory_allocation *)args->cookie;
	MALI_DEBUG_ASSERT_POINTER(descriptor);
	...
	maliggy_allocation_engine_release_pt1_maliggy_pagetables_unmap(memory_engine, descriptor);

	maliggy_scheduler_zap_all_active(session_data);

	/* Removes the descriptor from the session's memory list, releases physical memory, releases descriptor */
	maliggy_allocation_engine_release_pt2_physical_memory_free(memory_engine, descriptor);
...
{
	maliggy_memory_allocation * descriptor;
	_maliggy_osk_lock_t *descriptor_lock;
	_maliggy_osk_errcode_t err;
	...
	MALI_DEBUG_ASSERT_POINTER((struct maliggy_session_data *)descriptor->maliggy_addr_mapping_info);
	...
	while (err == _MALI_OSK_ERR_BUSY)
	{
		if (descriptor_lock)
		{
			_maliggy_osk_lock_wait( descriptor_lock, _MALI_OSK_LOCKMODE_RW );
		}

		err = _maliggy_ukk_mem_munmap_internal( args );

		if (descriptor_lock)
		{
			_maliggy_osk_lock_signal( descriptor_lock, _MALI_OSK_LOCKMODE_RW );
		}
...
{
	return maliggy_allocation_engine_memory_usage(physical_memory_allocators);
}

_maliggy_osk_errcode_t maliggy_mmu_get_table_page(u32 *table_page, maliggy_io_address *mapping)
{
	_maliggy_osk_lock_wait(page_table_cache.lock, _MALI_OSK_LOCKMODE_RW);

	if (!_maliggy_osk_list_empty(&page_table_cache.partial))
	{
		maliggy_mmu_page_table_allocation * alloc = _MALI_OSK_LIST_ENTRY(page_table_cache.partial.next, maliggy_mmu_page_table_allocation, list);
		int page_number = _maliggy_osk_find_first_zero_bit(alloc->usage_map, alloc->num_pages);
		MALI_DEBUG_PRINT(6, ("Partial page table allocation found, using page offset %d\n", page_number));
		_maliggy_osk_set_nonatomic_bit(page_number, alloc->usage_map);
		alloc->usage_count++;
		if (alloc->num_pages == alloc->usage_count)
		{
			/* full, move alloc to full list*/
			_maliggy_osk_list_move(&alloc->list, &page_table_cache.full);
		}
		_maliggy_osk_lock_signal(page_table_cache.lock, _MALI_OSK_LOCKMODE_RW);

		*table_page = (MALI_MMU_PAGE_SIZE * page_number) + alloc->pages.phys_base;
		*mapping =  (maliggy_io_address)((MALI_MMU_PAGE_SIZE * page_number) + (u32)alloc->pages.mapping);
		MALI_DEBUG_PRINT(4, ("Page table allocated for VA=0x%08X, MaliPA=0x%08X\n", *mapping, *table_page ));
		MALI_SUCCESS;
	}
	else
	{
		maliggy_mmu_page_table_allocation * alloc;
		/* no free pages, allocate a new one */

		alloc = (maliggy_mmu_page_table_allocation *)_maliggy_osk_calloc(1, sizeof(maliggy_mmu_page_table_allocation));
		if (NULL == alloc)
		{
			_maliggy_osk_lock_signal(page_table_cache.lock, _MALI_OSK_LOCKMODE_RW);
		...
		if (_MALI_OSK_ERR_OK != maliggy_allocation_engine_allocate_page_tables(memory_engine, &alloc->pages, physical_memory_allocators))
		{
			MALI_DEBUG_PRINT(1, ("No more memory for page tables\n"));
			_maliggy_osk_free(alloc);
			_maliggy_osk_lock_signal(page_table_cache.lock, _MALI_OSK_LOCKMODE_RW);
			*table_page = MALI_INVALID_PAGE;
			*mapping = NULL;
			MALI_ERROR(_MALI_OSK_ERR_NOMEM);
		...
		alloc->usage_map = _maliggy_osk_calloc(1, ((alloc->num_pages + BITS_PER_LONG - 1) & ~(BITS_PER_LONG-1) / BITS_PER_LONG) * sizeof(unsigned long));
		...
		_maliggy_osk_set_nonatomic_bit(0, alloc->usage_map);

		if (alloc->num_pages > 1)
		{
			_maliggy_osk_list_add(&alloc->list, &page_table_cache.partial);
		}
		else
		{
			_maliggy_osk_list_add(&alloc->list, &page_table_cache.full);
		}
...
{
	maliggy_mmu_page_table_allocation * alloc, * temp_alloc;

	MALI_DEBUG_PRINT_IF(1, pa & 4095, ("Bad page address 0x%x given to maliggy_mmu_release_table_page\n", (void*)pa));
	...
	_MALI_OSK_LIST_FOREACHENTRY(alloc, temp_alloc, &page_table_cache.partial, maliggy_mmu_page_table_allocation, list)
	{
		u32 start = alloc->pages.phys_base;
		u32 last = start + (alloc->num_pages - 1) * MALI_MMU_PAGE_SIZE;
		if (pa >= start && pa <= last)
		{
			MALI_DEBUG_ASSERT(0 != _maliggy_osk_test_bit((pa - start)/MALI_MMU_PAGE_SIZE, alloc->usage_map));
			_maliggy_osk_clear_nonatomic_bit((pa - start)/MALI_MMU_PAGE_SIZE, alloc->usage_map);
			alloc->usage_count--;

			_maliggy_osk_memset((void*)( ((u32)alloc->pages.mapping) + (pa - start) ), 0, MALI_MMU_PAGE_SIZE);

			if (0 == alloc->usage_count)
			{
				/* empty, release whole page alloc */
				_maliggy_osk_list_del(&alloc->list);
				alloc->pages.release(&alloc->pages);
				_maliggy_osk_free(alloc->usage_map);
				_maliggy_osk_free(alloc);
			}
		   	_maliggy_osk_lock_signal(page_table_cache.lock, _MALI_OSK_LOCKMODE_RW);
	...
	_MALI_OSK_LIST_FOREACHENTRY(alloc, temp_alloc, &page_table_cache.full, maliggy_mmu_page_table_allocation, list)
	{
		u32 start = alloc->pages.phys_base;
		u32 last = start + (alloc->num_pages - 1) * MALI_MMU_PAGE_SIZE;
		if (pa >= start && pa <= last)
		{
			...
			else
			{
				...
				_maliggy_osk_list_move(&alloc->list, &page_table_cache.partial);
			}
...
static _maliggy_osk_errcode_t maliggy_mmu_page_table_cache_create(void)
{
	page_table_cache.lock = _maliggy_osk_lock_init( _MALI_OSK_LOCKFLAG_ORDERED | _MALI_OSK_LOCKFLAG_ONELOCK
...
static void maliggy_mmu_page_table_cache_destroy(void)
{
	maliggy_mmu_page_table_allocation * alloc, *temp;

	_MALI_OSK_LIST_FOREACHENTRY(alloc, temp, &page_table_cache.partial, maliggy_mmu_page_table_allocation, list)
	{
	...
	MALI_DEBUG_PRINT_IF(1, !_maliggy_osk_list_empty(&page_table_cache.full), ("Page table cache full list contains one or more elements \n"));

	_MALI_OSK_LIST_FOREACHENTRY(alloc, temp, &page_table_cache.full, maliggy_mmu_page_table_allocation, list)
	{
	...
	_maliggy_osk_lock_term(page_table_cache.lock);
}
