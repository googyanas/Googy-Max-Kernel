...
static struct maliggy_pm_domain *maliggy_pm_domains[MALI_PM_DOMAIN_MAX_DOMAINS] = { NULL, };

static void maliggy_pm_domain_lock(struct maliggy_pm_domain *domain)
{
	_maliggy_osk_lock_wait(domain->lock, _MALI_OSK_LOCKMODE_RW);
}

static void maliggy_pm_domain_unlock(struct maliggy_pm_domain *domain)
{
	_maliggy_osk_lock_signal(domain->lock, _MALI_OSK_LOCKMODE_RW);
}

MALI_STATIC_INLINE void maliggy_pm_domain_state_set(struct maliggy_pm_domain *domain, maliggy_pm_domain_state state)
{
	domain->state = state;
}
...
struct maliggy_pm_domain *maliggy_pm_domain_create(u32 id, u32 pmu_mask)
{
	...
	domain = (struct maliggy_pm_domain *)_maliggy_osk_malloc(sizeof(struct maliggy_pm_domain));
	if (NULL != domain)
	{
		_maliggy_osk_lock_flags_t flags = _MALI_OSK_LOCKFLAG_SPINLOCK_IRQ | _MALI_OSK_LOCKFLAG_NONINTERRUPTABLE;
		domain->lock = _maliggy_osk_lock_init(flags, 0, _MALI_OSK_LOCK_ORDER_PM_DOMAIN);
		if (NULL == domain->lock)
		{
			_maliggy_osk_free(domain);
			return NULL;
		}
...
void maliggy_pm_domain_delete(struct maliggy_pm_domain *domain)
{
	...
	_maliggy_osk_lock_term(domain->lock);
...
void maliggy_pm_domain_terminate(void)
{
	...
	{
		maliggy_pm_domain_delete(maliggy_pm_domains[i]);
	}
}

void maliggy_pm_domain_add_group(u32 id, struct maliggy_group *group)
{
	struct maliggy_pm_domain *domain = maliggy_pm_domain_get(id);
	struct maliggy_group *next;
	...
	maliggy_pm_domain_ref_get(domain);
	...
	maliggy_group_set_pm_domain(group, domain);
}
...
void maliggy_pm_domain_add_l2(u32 id, struct maliggy_l2_cache_core *l2)
{
	...
	maliggy_l2_cache_set_pm_domain(l2, domain);
}
...
void maliggy_pm_domain_ref_get(struct maliggy_pm_domain *domain)
{
	...
	if (MALI_PM_DOMAIN_ON != domain->state)
	{
		...
		if (NULL != pmu)
		{
			...
			err = maliggy_pmu_power_up(pmu, domain->pmu_mask);
		...
		maliggy_pm_domain_state_set(domain, MALI_PM_DOMAIN_ON);
	}
	else
	{
		MALI_DEBUG_ASSERT(MALI_PM_DOMAIN_ON == maliggy_pm_domain_state_get(domain));
	}

	maliggy_pm_domain_unlock(domain);
}
...
void maliggy_pm_domain_ref_put(struct maliggy_pm_domain *domain)
{
	...
	if (0 == domain->use_count && MALI_PM_DOMAIN_OFF != domain->state)
	{
		...
		maliggy_pm_domain_state_set(domain, MALI_PM_DOMAIN_OFF);

		if (NULL != pmu)
		{
			_maliggy_osk_errcode_t err;

			err = maliggy_pmu_power_down(pmu, domain->pmu_mask);
...
{
	maliggy_bool is_powered = MALI_TRUE;
	...
	if(!_maliggy_osk_pm_dev_ref_add_no_power_on())
	{
		is_powered = MALI_FALSE;
	}
...
void maliggy_pm_domain_unlock_state(struct maliggy_pm_domain *domain)
{
	...
	if (NULL != domain)
	{
		maliggy_pm_domain_ref_put(domain);
	}
}
