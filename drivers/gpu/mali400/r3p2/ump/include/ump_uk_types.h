...
} umpggy_uk_cache_op_control;

typedef enum
{
	_UMP_UK_READ = 1,
	_UMP_UK_READ_WRITE = 3,
} umpggy_uk_lock_usage;
 ...
 */
typedef struct _umpggy_uk_api_version_s
{
...
} _umpggy_uk_api_version_s;

/**
 * ALLOCATE ([out] u32 secure_id, [in,out] u32 size,  [in] contraints)
 */
typedef struct _umpggy_uk_allocate_s
{
	void *ctx;                              /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;                          /**< Return value from DD to Userdriver */
	u32 size;                               /**< Input and output. Requested size; input. Returned size; output */
	umpggy_uk_alloc_constraints constraints;   /**< Only input to Devicedriver */
} _umpggy_uk_allocate_s;

typedef struct _umpggy_uk_ion_import_s
{
	void *ctx;                              /**< [in,out] user-kernel context (trashed on output) */
	int ion_fd;                             /**< ion_fd */
	u32 secure_id;                          /**< Return value from DD to Userdriver */
	u32 size;                               /**< Input and output. Requested size; input. Returned size; output */
	umpggy_uk_alloc_constraints constraints;   /**< Only input to Devicedriver */
} _umpggy_uk_ion_import_s;

/**
 * SIZE_GET ([in] u32 secure_id, [out]size )
 */
typedef struct _umpggy_uk_size_get_s
{
	void *ctx;                              /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;                          /**< Input to DD */
	u32 size;                               /**< Returned size; output */
} _umpggy_uk_size_get_s;

/**
 * Release ([in] u32 secure_id)
 */
typedef struct _umpggy_uk_release_s
{
	void *ctx;                              /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;                          /**< Input to DD */
} _umpggy_uk_release_s;

typedef struct _umpggy_uk_map_mem_s
{
	void *ctx;                      /**< [in,out] user-kernel context (trashed on output) */
...
} _umpggy_uk_map_mem_s;

typedef struct _umpggy_uk_unmap_mem_s
{
	void *ctx;            /**< [in,out] user-kernel context (trashed on output) */
	void *mapping;
	u32 size;
	void * _ukk_private;
	u32 cookie;
} _umpggy_uk_unmap_mem_s;

typedef struct _umpggy_uk_msync_s
{
	void *ctx;            /**< [in,out] user-kernel context (trashed on output) */
	void *mapping;        /**< [in] mapping addr */
	void *address;        /**< [in] flush start addr */
	u32 size;             /**< [in] size to flush */
	umpggy_uk_msync_op op;   /**< [in] flush operation */
...
typedef struct _umpggy_uk_cache_operations_control_s
{
	void *ctx;                   /**< [in,out] user-kernel context (trashed on output) */
	umpggy_uk_cache_op_control op;  /**< [in] cache operations start/stop */
} _umpggy_uk_cache_operations_control_s;


typedef struct _umpggy_uk_switch_hw_usage_s
{
	void *ctx;            /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;        /**< [in] secure_id that identifies the ump buffer */
	umpggy_uk_user new_user;         /**< [in] cookie stored with reference to the kernel mapping internals */

} _umpggy_uk_switch_hw_usage_s;

typedef struct _umpggy_uk_lock_s
{
	void *ctx;            /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;        /**< [in] secure_id that identifies the ump buffer */
	umpggy_uk_lock_usage lock_usage;
} _umpggy_uk_lock_s;

typedef struct _umpggy_uk_unlock_s
{
	void *ctx;            /**< [in,out] user-kernel context (trashed on output) */
	u32 secure_id;        /**< [in] secure_id that identifies the ump buffer */
} _umpggy_uk_unlock_s;
